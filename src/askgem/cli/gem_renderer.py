...
class GemStyleRenderer:
    ...
    def _build_view(self, show_cursor: bool = True) -> Group:
        """Construct a Group with all committed content + live streaming text."""
    ...
    def end_stream(self, full_text: str | None = None) -> None:
        ...
        # Print the definitive final view of this turn part
        self.console.print(self._build_view(show_cursor=False))
    ...
    def print_tool_result(self, ok: bool, content: str, tool_name: str | None = None) -> None:
        ...
        is_list = any(l.strip().startswith(("-", "*", "1.", " •")) for l in lines[:10])
        is_diff = content.strip().startswith(("---", "+++", "@@"))

        # Expand if it's a list, diff, or short structured content (up to 25 lines)
        if ok and len(lines) <= 25 and (is_list or is_diff or len(content) < 800):
            ...
            if is_diff:
                ...
                preview_renderable = Markdown(content)
            else:
            ...
            if len(content) > 120:
                preview += "..."
            line = Text.from_markup(f"  {icon} [bold]{name_display}[/] [dim]({artifact_id})[/]  [dim]{escape(preview)}[/]")
    ...
    def expand_artifact(self, index: int = -1) -> None:
        ...
        try:
            ...
            if name in ["read_file", "edit_file", "write_file", "execute_bash"]:
                renderable = Syntax(content, "bash" if "bash" in name else "python", theme="monokai", line_numbers=True)
            elif "[LSP DIAGNOSTICS" in content:
    ...
    def print_welcome(self, version: str, model: str, mode: str) -> None:
        self.console.print()
        self.console.print(
            f"  [bold {self.C_BRAND}]{icons.brand} AskGem[/]  [dim]v{version}[/]  "
