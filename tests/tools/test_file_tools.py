...
from askgem.tools.file_tools import ensure_safe_path, edit_file, read_file
...
class TestEditFile:
    ...
    def test_creates_bkp_before_editing(self, tmp_path):
        with patch("askgem.tools.file_tools._create_backup", return_value=str(tmp_path / "code.py.bkp")) as mock_backup:
            mock_backup.side_effect = lambda p: (tmp_path / "code.py.bkp").write_text("original content") and str(tmp_path / "code.py.bkp")
            f = tmp_path / "code.py"
